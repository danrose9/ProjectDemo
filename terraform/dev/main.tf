...
resource "azurerm_key_vault_certificate" "res-6" {
...
resource "azurerm_key_vault_key" "res-7" {
...
resource "azurerm_key_vault_secret" "res-9" {
  key_vault_id = azurerm_key_vault.res-5.id
  name         = "JwtIssuer"
...
resource "azurerm_log_analytics_saved_search" "res-11" {
  ...
  log_analytics_workspace_id = azurerm_log_analytics_workspace.res-707.id
...
resource "azurerm_mssql_server" "res-642" {
...
resource "azurerm_mssql_database" "res-654" {
  name                 = "ProjectDemoSql"
  server_id            = azurerm_mssql_server.res-642.id
  storage_account_type = "Local"
  tags = {
    project = "project-demo"
  }
}
resource "azurerm_mssql_database_extended_auditing_policy" "res-660" {
  database_id            = azurerm_mssql_database.res-654.id
  enabled                = false
  log_monitoring_enabled = false
}
resource "azurerm_mssql_database_extended_auditing_policy" "res-669" {
...
resource "azurerm_mssql_server_microsoft_support_auditing_policy" "res-675" {
  enabled                = false
  log_monitoring_enabled = false
  server_id              = azurerm_mssql_server.res-642.id
}
resource "azurerm_mssql_server_transparent_data_encryption" "res-676" {
  server_id = azurerm_mssql_server.res-642.id
}
resource "azurerm_mssql_server_extended_auditing_policy" "res-677" {
  enabled                = false
  log_monitoring_enabled = false
  server_id              = azurerm_mssql_server.res-642.id
}
resource "azurerm_mssql_firewall_rule" "res-678" {
  end_ip_address   = "95.166.23.32"
  name             = "ClientIPAddress_2025-6-23_22-2-26"
  server_id        = azurerm_mssql_server.res-642.id
  start_ip_address = "95.166.23.32"
}
resource "azurerm_mssql_server_security_alert_policy" "res-680" {
  resource_group_name = "RG-ProjectDemo-Dev"
  server_name         = "project-demo-sql-server"
  state               = "Disabled"
  depends_on = [
    azurerm_mssql_server.res-642
  ]
}
resource "azurerm_mssql_server_vulnerability_assessment" "res-682" {
  server_security_alert_policy_id = azurerm_mssql_server_security_alert_policy.res-680.id
  storage_container_path          = ""
}
resource "azurerm_storage_account" "res-683" {
...
resource "azurerm_storage_container" "res-685" {
  name               = "tfstate"
  ...
  depends_on = [
    # One of azurerm_storage_account.res-683,azurerm_storage_account_queue_properties.res-687 (can't auto-resolve as their ids are identical)
  ]
}
resource "azurerm_storage_account_queue_properties" "res-687" {
  storage_account_id = azurerm_storage_account.res-683.id
  hour_metrics {
    version = "1.0"
  }
  logging {
...
resource "azurerm_service_plan" "res-689" {
  location            = "eastus2"
...
resource "azurerm_windows_web_app" "res-690" {
  app_settings = {
  ...
  name                                     = "ProjectDemoApi"
  resource_group_name                      = "RG-ProjectDemo-Dev"
  service_plan_id                          = azurerm_service_plan.res-689.id
  tags = {
...
resource "azurerm_app_service_custom_hostname_binding" "res-704" {
  app_service_name    = "ProjectDemoApi"
  hostname            = "projectdemoapi-hfdecyeea8gyacbg.eastus2-01.azurewebsites.net"
  resource_group_name = "RG-ProjectDemo-Dev"
  depends_on = [
    azurerm_windows_web_app.res-690
  ]
}
resource "azurerm_monitor_action_group" "res-705" {
...
resource "azurerm_application_insights" "res-706" {
...
resource "azurerm_log_analytics_workspace" "res-707" {
  location            = "eastus"
